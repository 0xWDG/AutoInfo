...
- (NSString *) encode:(NSString*)string1 salt:(NSString *)string2
{
    ...
    {
        new = [[NSString alloc] initWithFormat:@"%@%@%@",
               new,
               [string1 substringWithRange:NSMakeRange(i, 1)],
               [string2 substringWithRange:NSMakeRange(i, 1)]
               ];
    }
    ...
    if ((self = [super init]))
        {
            ...
            if (![
                    [[NSString alloc] initWithFormat:@"%@", [self get:@"Psp6F(wE6(ZiuI27D8_Gifjxnc-+NuAvW_CuV00-(ZcqAfy6nqMGG~6)a<YYtqmhi000"]]
                    isEqualToString:@"q_CFRPamU<SG8m01wFcbm774CI4SgexH71eYRpdDp>bxkczRRL+5S2anLe7-tEABG000"
                ])
                [self makeConfig];
...
{
    if (![
            [[NSString alloc] initWithFormat:@"%@", [configdb allValues]] isEqualToString:@"(null)"
          ])
    {
    NSArray *paths                  = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString *documentsDirectory    = [paths objectAtIndex:0];
    NSString *path                  = [documentsDirectory stringByAppendingPathComponent:saveFile];
    NSError *error                  = nil;
    //NSLog(@"%@ -- %@", [configdb allKeys], [configdb allValues]);
